...
export async function scheda(req:Request, res:Response) {
    connection.execute(
        `SELECT film.idfilm, film.titolo, film.locandina, film.regista, film.anno, film.descrizione, p.idproiezione, p.datap, p.orario FROM film JOIN proiezioni p ON film.idfilm = p.idfilm WHERE film.idfilm=?`, 
        [req.params.id],
        function (err, results, fields){
            console.log(results)
            res.json(results)
